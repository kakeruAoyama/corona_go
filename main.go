...
func main() {
	...
	router.GET("/hello", func(ctx *gin.Context) {
		...
		back_1day := dayBack(-1, date)
		back_1day_cumulative_value := fetchApi(back_1day, encoded_prefecture)
		difference_1days := subtractStrings(cumulative_value[0].Npatients, back_1day_cumulative_value[0].Npatients)

		back_7day := dayBack(-7, date)
		back_7day_cumulative_value := fetchApi(back_7day, encoded_prefecture)
		difference_7days := subtractStrings(cumulative_value[0].Npatients, back_7day_cumulative_value[0].Npatients)
...
func dayBack(back_count int, string_date string) string {
...
func subtractStrings(before string, after string) int {
